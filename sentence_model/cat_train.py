...
from model import sentNetClusteredCat
import numpy as np 
...
userDirs=['../ASL_sentences/ali_33sents']
#users=['riley','ari','arisa','jesse','ali']
#users=['riley','ari','arisa','jesse']
users=['ali']
testCount=20
outFile='train_test_ali_18'
...
filePath='/home/psanthal/impactProject/sentence_model/train_test_four_ali'
...
exit(0)
logger=createLogger('/scratch/psanthal','unclustered-sent-all-18')
logger.info("Training set total number of samples:%s",len(trainDataset))
saveDir='/scratch/psanthal/unclustered-sent-all-18/'
torch.manual_seed(1)
...
multiViewDataLoader=DataLoader(trainDataset,6,shuffle=True)
