...
def cloudPointVisualize(inFile,inProximity=.05,pltType='3d',saveDir=None,saveNumpy=False):
	inFile=open(inFile,'r')
	try:
		lines=inFile.readlines()
	except:
...
def perProcessCall(inFiles,saveDirs,func,currArgs):
	for i,inFile in enumerate(inFiles):
		if i==0:
			currArgs.insert(0,inFile)
		else:
			currArgs[0]=inFile
		currArgs[3]=saveDirs[i]
		args=tuple(currArgs)
		func(*args)

def processFiles(inDir,saveDir,distances,kinectFile,Multiview=False,azimuths=None,elevations=None,writeToFile=False,anglePairs=None,separateHands=False,reSize=False,saveNumpy=False,cluster=True,FileName=None):
	...
	for inFile in inFiles:
		...
		if not writeToFile:
			...
			saveDirs.append(user+'_'+dirName)
		if Multiview:
			if anglePairs:
			...
			if not saveNumpy and not writeToFile:
				for plane in planes:
					try:
	...
	for i in range(0,len(inFiles),fileCount):
		if writeToFile:
			currArgs=[distances, kinectFile, 'tempDir', Multiview, azimuths, elevations, writeToFile, FileName,anglePairs]
			p = mp.Process(target=perProcessCall, args=(inFiles[i:i+fileCount],saveDirs[i:i+fileCount],funcName,currArgs))
			if count > 0:
				for k in processes:
					k.join()
					processes = []
			...
			if cluster:
				currArgs=[distances,kinectFile,'tempDir',Multiview,azimuths,elevations,False,None,anglePairs,separateHands,reSize,saveNumpy]
			else:
				currArgs=[.05,'2d','tempDir',True]
			p=mp.Process(target=perProcessCall,args=(inFiles[i:i+fileCount],saveDirs[i:i+fileCount],funcName,currArgs))
...
def saveFigure(x,pltTitle='temp',clustered=False,saveDir=None,pltType='3d',axis=None,Multiview=False,elevations=None,azimuths=None,anglePairs=None,writetoFile=False,FileName=None,separateHands=False,reSize=False,forPart=None,saveNumpy=False):
	...
	if pltType=='3d':
		if writetoFile:
			if any([key is None for key in x.keys()]):
				return
			for key in x.keys():
		...
		if clustered:
			...
			if x is None:
				...
				ax.scatter(x[:,i],x[:,j],c='red',marker='o')
			if separateHands:
	...
	for line in lines:
		...
		if currTime is None:
				...
				if len(procClusters) > 2:
					...
					if saveDir is not None and not Multiview and not writetoFile:
						if noBody:
							for axis in axes:
								if saveNumpy:
									npArrays[axis].append(saveFigure(toPlot,pltTitle='Image-'+str(count),clustered=True,saveDir=saveDir+'/'+axis,pltType='2d',axis=axis,separateHands=separateHands,reSize=reSize,forPart=None,saveNumpy=saveNumpy))
						else:
							for axis in axes:
								for part in bodyParts:
									if saveNumpy:
										npArrays[axis+'-'+part].append(saveFigure(toPlot[part],pltTitle='Image-'+str(count),clustered=False,saveDir=saveDir+'/'+axis,pltType='2d',axis=axis,separateHands=separateHands,reSize=reSize,forPart=part,saveNumpy=saveNumpy))
					elif Multiview:
						...
						if toPlot['body'] is not None:
							data_body.append(','.join([str(x) for x in np.mean(toPlot['body'],axis=0).tolist()]))
						else:
							data_body.append('0.0,0.0,0.0')
						if toPlot['left'] is not None:
							data_left.append(','.join([str(x) for x in np.mean(toPlot['left'],axis=0).tolist()]))
						else:
							data_left.append('0.0,0.0,0.0')
						if toPlot['right'] is not None:
							data_right.append(','.join([str(x) for x in np.mean(toPlot['right'],axis=0).tolist()]))
						else:
							data_right.append('0.0,0.0,0.0')
			...
			if line[0] <= 1.5 and line[0] >= 0.2 and line[1] <= radius and line[1] >=-radius and line[2]<=radius and line[2]>=-radius:
				...
				data_z.append(line[2])
	if writetoFile:
		print(','.join(data_body)+','+'body'+','+saveDir)
		print(','.join(data_body)+','+'left'+','+saveDir)
		print(','.join(data_body)+','+'right'+','+saveDir)
		FileName.write(','.join(data_body)+','+'body'+','+saveDir+'\n')
		FileName.write(','.join(data_left)+','+'left'+','+saveDir+'\n')
		FileName.write(','.join(data_right)+','+'right'+','+saveDir+'\n')
	if saveNumpy:
		if noBody:
			for axis in axes:
				np.save(saveDir+'/'+axis,npArrays[axis])
			print("Done saving")
		else:
			for axis in axes:
				for part in bodyParts:
...
inDir='/home/psanthal/impactProject/riley_new23words'
saveDir='/scratch/psanthal/impactProject/unclustered_words'
# cloudPointVisualize('ali_23words/ali_huddle_02.txt',0.05,0,'Gesture',saveDir='temp',pltType='3d',axis='yz')
cloudPointVisualize('./how_you_forward_01.txt',0.05,None,saveDir='./temp')

distances=[0.05,0.075,0.1,.125,.15,.175,.2]
dist=.075
elevations=[0,90,180,270,360]
azimuths=[0,45,90,135,180,225,270,315,360]
anglePairs=[(240,33),(37,33),(119,33)]
...
kinectFile='/home/psanthal/impactProject/primitive_motions'
...
#getConvexHull(None)
#write_file='/scratch/psanthal/impactProject/Jesse_centroidData'
processFiles(inDir,saveDir,distances,kinectFile,False,separateHands=True,reSize=True,saveNumpy=True,cluster=False)
