...
trainDataset=multiViewDatasetConcat(dirPath,classes,filePath,train=True,bodySeparate=True,frameCount=40,wordOnly=True)
logger=createLogger('/scratch/psanthal','separated-word-all-2')
logger.info("Training set total number of samples:%s",len(trainDataset))
saveDir='/scratch/psanthal/separated-word-all-2/'
torch.manual_seed(1)
...
for epoch in range(400):
	...
	for x,label in multiViewDataLoader:		
		net.zero_grad()
		y=net(x.cuda(),40)
		loss=criterion(y,label.to('cuda:2'))
		loss.backward()
		optimizer.step()
